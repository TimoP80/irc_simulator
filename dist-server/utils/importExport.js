    ...
    const csvContent = [
        ...
        ...users.map(user => {
            ...
            if (isPerLanguageFormat(user.languageSkills)) {
                ...
                languagesString = user.languageSkills.languages.map(lang => `${lang}:${user.languageSkills.fluency || 'native'}:${user.languageSkills.accent || ''}`).join(';');
            }
            else {
...
export const importUsersFromCSV = (csvContent) => {
    ...
    for (let i = 1; i < lines.length; i++) {
        ...
        const user = {
            ...
            languageSkills: {
                languages: languages.length > 0 ? languages.map(l => ({ ...l, fluency: l.fluency })) : [{ language: 'English', fluency: 'native', accent: '' }]
            },
            writingStyle: {
                formality: values[3] || 'semi_formal',
                verbosity: values[4] || 'moderate',
                humor: values[5] || 'none',
                emojiUsage: values[6] || 'rare',
                punctuation: values[7] || 'standard'
            }
        };
...
export const exportUsersToJSON = (users) => {
        ...
        if (Array.isArray(data)) {
            return data.map(user => {
                ...
                if (Array.isArray(user.languageSkills)) {
                    ...
                    languageSkills = {
                        languages: user.languageSkills.map((lang) => ({
                    ...
                    if (isPerLanguageFormat(user.languageSkills)) {
                        ...
                        const languages = user.languageSkills.languages;
                ...
                if (user.writingStyle && typeof user.writingStyle === 'object') {
                    ...
                    const convertVerbosity = (verbosity) => {
                        if (isStationVFormat(verbosity))
                            return verbosity;
                        const mapping = {
                    ...
                    const convertHumor = (humor) => {
                        if (isStationVFormat(humor))
                            return humor;
                        const mapping = {
                            'None': 'none',
                            'Dry': 'dry',
                    ...
                    const convertEmojiUsage = (emojiUsage) => {
                        if (isStationVFormat(emojiUsage))
                            return emojiUsage;
                        const mapping = {
                            'None': 'none',
                            'Low': 'low',
                    ...
                    const convertPunctuation = (punctuation) => {
                        if (isStationVFormat(punctuation))
                            return punctuation;
                        const mapping = {
                            'Minimal': 'minimal',
                            'Standard': 'standard',
...
export const exportChannelToHTML = (channel, currentUserNickname) => {
...
export const exportAllChannelsToHTML = (channels, currentUserNickname) => {
