...
export class DataExportService {
    ...
    async exportAllData() {
        ...
        try {
            ...
            const channels = channelData ? JSON.parse(channelData) : [];
            // Load private messages
            const pmData = localStorage.getItem('station-v-private-messages');
            const privateMessages = pmData ? JSON.parse(pmData) : {};
            // Load unread status
            const unreadPMUsersData = localStorage.getItem('station-v-unread-pm-users');
            const unreadPMUsers = unreadPMUsersData ? JSON.parse(unreadPMUsersData) : [];
            const unreadChannelsData = localStorage.getItem('station-v-unread-channels');
            const unreadChannels = unreadChannelsData ? JSON.parse(unreadChannelsData) : [];
            // Load active context
