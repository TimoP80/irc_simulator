...
export const handleBotCommand = async (command, user, channelName, model = 'gemini-2.5-flash', imageConfig, addMessageToContext, updateMessageInContext, generateUniqueMessageId, activeContext) => {
...
const generateImageCommand = async (args, user, channelName, model, imageConfig, addMessageToContext, updateMessageInContext, generateUniqueMessageId, activeContext) => {
...
// Weather information
const generateWeatherCommand = async (args, user, channelName, model) => {
...
// Current time
const generateTimeCommand = async (user, channelName, model) => {
    const now = new Date();
...
// Information lookup
const generateInfoCommand = async (args, user, channelName, model) => {
...
// Help command
const generateHelpCommand = async (user, channelName, model) => {
...
// Random quote
const generateQuoteCommand = async (user, channelName, model) => {
    try {
        const response = await withRateLimitAndRetries(async () => {
            return await ai.models.generateContent({
                model: model,
...
// Joke
const generateJokeCommand = async (user, channelName, model) => {
    try {
        const response = await withRateLimitAndRetries(async () => {
            return await ai.models.generateContent({
                model: model,
...
// Interesting fact
const generateFactCommand = async (user, channelName, model) => {
    try {
        const response = await withRateLimitAndRetries(async () => {
            return await ai.models.generateContent({
                model: model,
...
// Translation
const generateTranslateCommand = async (args, user, channelName, model) => {
...
// Calculator
const generateCalcCommand = async (args, user, channelName, model) => {
...
// Search
const generateSearchCommand = async (args, user, channelName, model) => {
...
export const isBotCommand = (content) => {
    const trimmed = content.trim();
    return trimmed.startsWith('!') && trimmed.length > 1;
};
// Get available bot commands
export const getAvailableBotCommands = () => {
    return [
