...
export const initializeRelationshipMemory = (user) => {
...
export const updateRelationshipMemory = (aiUser, otherUserNickname, channel, message, interactionType = 'message_exchange') => {
...
const calculateRelationshipLevel = (relationship) => {
    ...
    if (daysSinceLastInteraction > 7) {
        if (relationship.relationshipLevel === 'close')
            return 'friendly';
        if (relationship.relationshipLevel === 'friendly')
            return 'acquaintance';
        if (relationship.relationshipLevel === 'acquaintance')
            return 'stranger';
    }
    // Relationship progression based on interaction count and frequency
    if (interactionCount >= 50 && daysSinceFirstMet >= 3)
        return 'close';
    if (interactionCount >= 20 && daysSinceFirstMet >= 1)
        return 'friendly';
...
export const getRelationshipContext = (aiUser, otherUserNickname, currentChannel) => {
...
export const updateChannelRelationshipMemory = (users, message, channel) => {
    return users.map(user => {
        // Only update memory for virtual users
        if (user.userType !== 'virtual')
            return user;
...
export const getRelationshipSummary = (user) => {
    const memory = user.relationshipMemory;
    if (!memory)
        return 'No relationship memory';
    const relationships = Object.values(memory.relationships);
    const summary = relationships.map(rel => `${rel.nickname}: ${rel.relationshipLevel} (${rel.interactionCount} interactions)`).join(', ');
