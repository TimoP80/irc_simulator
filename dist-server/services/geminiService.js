...
const formatEnhancedMessageHistory = (messages) => {
...
const extractLinksAndImages = (content) => {
...
const safeGetUserProperty = (user, property, fallback = null) => {
    if (!user)
        return fallback;
    switch (property) {
        case 'personality':
...
const getLanguageExamples = (language) => {
    const examples = {
...
const getTimeOfDayContext = () => {
    ...
    let specialContext = '';
    if (month === 0 && day === 1)
        specialContext = 'New Year\'s Day - people are making resolutions and reflecting on the past year';
    else if (month === 1 && day === 14)
        specialContext = 'Valentine\'s Day - romantic discussions and relationship topics are common';
    else if (month === 2 && day === 17)
        specialContext = 'St. Patrick\'s Day - Irish celebrations and green themes';
    else if (month === 3 && day === 1)
        specialContext = 'April Fool\'s Day - pranks and jokes are popular';
    else if (month === 4 && day === 1)
        specialContext = 'May Day - spring celebrations and workers\' rights';
    else if (month === 5 && day === 1)
        specialContext = 'Children\'s Day in many countries - family and child-related discussions';
    else if (month === 6 && day === 4)
        specialContext = 'Independence Day in the US - patriotic discussions and fireworks';
    else if (month === 9 && day === 31)
        specialContext = 'Halloween - costume discussions and spooky themes';
    else if (month === 10 && day === 11)
        specialContext = 'Veterans Day - military appreciation and service discussions';
    else if (month === 10 && day === 25)
        specialContext = 'Thanksgiving - family gatherings and gratitude discussions';
    else if (month === 11 && day === 25)
        specialContext = 'Christmas Day - holiday celebrations and gift discussions';
    else if (month === 11 && day === 31)
        specialContext = 'New Year\'s Eve - year-end reflections and party planning';
...
export const generateChannelActivity = async (channel, currentUserNickname, model = 'gemini-2.5-flash', addMessageToContext, updateMessageInContext, generateUniqueMessageId, activeContext) => {
    ...
    if (afterhoursActive) {
        ...
        const nocturnalUsers = shuffledUsers.filter(user => (user.personality && user.personality.toLowerCase().includes('creative')) ||
        ...
        const energeticUsers = shuffledUsers.filter(user => (user.personality && user.personality.toLowerCase().includes('energetic')) ||
        ...
        const introspectiveUsers = shuffledUsers.filter(user => (user.personality && user.personality.toLowerCase().includes('quiet')) ||
...
const buildContextualPrompt = (context, aiUser, currentUserNickname, userMessage, timeContext, primaryLanguage, userLanguages, writingStyle) => {
...
const generateConversationSummary = (messages, currentUserNickname) => {
...
const analyzeConversationContext = (messages, currentUserNickname) => {
    ...
    let conversationMomentum = 'maintaining';
    if (recentActivity > olderActivity + 2)
        conversationMomentum = 'building';
    else if (recentActivity < olderActivity - 2)
...
export const generatePrivateMessageResponse = async (conversation, userMessage, currentUserNickname, model = 'gemini-2.5-flash', addMessageToContext, updateMessageInContext, generateUniqueMessageId, activeContext) => {
...
export const generateOperatorResponse = async (channel, requestingUser, operator, model = 'gemini-2.5-flash', addMessageToContext, updateMessageInContext, generateUniqueMessageId, activeContext) => {
    ...
    const messageHistory = recentMessages.map(msg => `${msg.nickname}: ${msg.content}`).join('\n');
...
export const generateBatchUsers = async (count, model = 'gemini-2.5-flash', options) => {
...
`;
    try {
        ...
        const response = await withRateLimitAndRetries(() => ai.models.generateContent({
            ...
            contents: prompt,
            config: config,
        }), `batch user generation (${count} users)`, { maxRetries: 1, initialBackoffMs: 500 });
...
export const generateRandomWorldConfiguration = async (model = 'gemini-2.5-flash') => {
    const validatedModel = validateModelId(model);
...
`;
    ...
    const response = await withRateLimitAndRetries(() => ai.models.generateContent({
        ...
        contents: prompt,
        config: config,
    }), `world configuration generation`, { maxRetries: 1, initialBackoffMs: 500 });
    const jsonString = extractTextFromResponse(response);
 ...
 */
export const generateUsername = async (style, avoidDuplicates = []) => {
 ...
 */
export async function generateTranslatedPersonality(personality, language) {
...
export const generateInCharacterComment = async (user, analysis, model = 'gemini-2.5-flash') => {
    const validatedModel = validateModelId(model);
