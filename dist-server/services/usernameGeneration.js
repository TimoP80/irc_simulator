...
export const USERNAME_CATEGORIES = [
    {
        id: 'tech',
 ...
 */
export const generateAUsernames = async (options) => {
 ...
 */
export const generateUsernamesForPersonality = async (personality, count, avoidDuplicates = []) => {
 ...
 */
export const generateBatchUsernames = async (count, existingUsers = []) => {
    const avoidDuplicates = existingUsers.map(u => u.toLowerCase());
    // Mix different styles for variety
    const styles = ['tech', 'gaming', 'creative', 'realistic', 'mixed'];
    const usernames = [];
 ...
 */
function generateFallbackUsernames(count, avoidDuplicates = []) {
    const prefixes = [
