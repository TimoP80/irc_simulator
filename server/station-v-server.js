...
// Import irc-framework using require for CommonJS compatibility
const ircFramework = require('irc-framework');
const { Server } = ircFramework;
...
class StationVServer extends EventEmitter {
  ...
  start(port = 8080) {
    ...
    // Create IRC server for IRC clients
    this.ircServer = new Server();
  ...
  setupIRCServer() {
    this.ircServer.on('client connection', (client) => {
      console.log('🔗 IRC client connected:', client.nick);
      
      client.on('register', () => {
        console.log('✅ IRC client registered:', client.nick);
        this.handleIRCClientRegister(client);
      });

      client.on('message', (event) => {
        console.log('📨 IRC message from', client.nick, 'in', event.target, ':', event.message);
        this.handleIRCMessage(client, event);
      });

      client.on('join', (event) => {
        console.log('📢 IRC client joined channel:', event.channel);
        this.handleIRCJoin(client, event);
      });

      client.on('part', (event) => {
        console.log('📢 IRC client left channel:', event.channel);
        this.handleIRCPart(client, event);
      });

      client.on('quit', () => {
        console.log('📢 IRC client quit:', client.nick);
        this.handleIRCQuit(client);
      });

      client.on('disconnect', () => {
        console.log('🔌 IRC client disconnected:', client.nick);
        this.handleIRCDisconnect(client);
      });
    });
  }
